...
export async function validateUrl(input) {
  ...
  // 3. Reject obviously internal hostnames
  ...
  // 4. DNS resolve and check for private IPs
  try {
    ...
    if (err.code === 'ENOTFOUND') {
      return { valid: false, reason: `DNS lookup failed: hostname "${hostname}" could not be resolved` };
    }
    return { valid: false, reason: `DNS resolution error: ${err.message}` };
  }
