...
function calculateCategoryScore(toolResults) {
  ...
  let score = 70; // base
  if (toolResults.title) score += 5;
  if (toolResults.metaDescription) score += 5;
  if (toolResults.viewport) score += 5;
  if (toolResults.canonical) score += 5;
  if (toolResults.tls && !toolResults.tls.expired) score += 5;
  if (toolResults.headers?.hsts) score += 5;
