...
export function listAudits(limit = 50, userId = null) {
  const db = getDb();
  let rows;
  if (userId) {
    rows = db.prepare('SELECT * FROM audits WHERE user_id = ? ORDER BY created_at DESC LIMIT ?').all(userId, limit);
  } else {
    rows = db.prepare('SELECT * FROM audits ORDER BY created_at DESC LIMIT ?').all(limit);
  }
  return rows.map((row) => {
    if (row.results) {
      row.results = JSON.parse(row.results);
